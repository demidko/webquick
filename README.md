...
3. [Облачный FIX-коннектор](https://arqatech.com/ru/products/quik/modules/integration-solutions/fix-software-interfaces/) для QUICK: так можно воспользоваться готовыми западными библиотеками для работы с FIX протоколом, однако при этом мы получаем сложное проксирование (от клиента к коннектору, от коннектора к терминалу, от терминала к брокеру, от брокера к бирже), стоит это дороже выделенного шлюза, и на мой взгляд, совершенно нерентабельно.
